...
static unsigned long __init e820_end_pfn(unsigned long limit_pfn, enum e820_type type)
{
	...
	pr_info("e820(type: %d) last_pfn = %#lx max_arch_pfn = %#lx\n",
			type, last_pfn, max_arch_pfn);
	return last_pfn;
}
...
//	unsigned long last_pfn = 0;
	...
	for (i = 0; i < e820_table->nr_entries; i++) {
		...
		start_pfn_pram = entry->addr >> PAGE_SHIFT;
		end_pfn_pram = (entry->addr + entry->size) >> PAGE_SHIFT;
...
{
	return e820_end_pfn(MAX_ARCH_PFN, E820_TYPE_PRAM);
}
