...
extern void wakeup_mbs_mntrd(struct zone *zone, int order, enum zone_type classzone_idx);
...
static struct mminit_pfnnid_cache early_pfnnid_cache_pram __meminitdata;
 ...
 * -- nyc
 */
#define expand_pram	expand
#if 0
static inline void expand_pram(struct zone *zone, struct page *page,
 ...
 */
static inline
struct page *__rmqueue_pram_smallest(struct zone *zone, unsigned int order,
						int migratetype)
{
	...
	for (current_order = order; current_order < MAX_ORDER; ++current_order) {
		...
		rmv_page_order(page);
		area->nr_free--;
		expand_pram(zone, page, order, current_order, area, migratetype);
 ...
 */
static struct page *__rmqueue_pram(struct zone *zone, unsigned int order,
				int migratetype)
{
	struct page *page;

retry:
	page = __rmqueue_pram_smallest(zone, order, migratetype);
	if (unlikely(!page)) {
#if 0
 ...
 */
static int rmqueue_pram_bulk(struct zone *zone, unsigned int order,
			...
			int migratetype, bool cold)
{
	...
	spin_lock(&zone->lock);
	for (i = 0; i < count; ++i) {
		struct page *page = __rmqueue_pram(zone, order, migratetype);
	...
	//__mod_zone_page_state(zone, NR_FREE_PRAMS, -(i << order));
	spin_unlock(&zone->lock);
	return alloced;
}
 ...
 */
static inline void pram_zone_statistics(struct zone *preferred_zone, struct zone *z)
{
#ifdef CONFIG_NUMA
			...
			struct list_head *list)
{
	...
	do {
		if (list_empty(list)) {
			pcp->count += rmqueue_pram_bulk(zone, 0,
					pcp->batch, list,
					migratetype, cold);
			...
			gfp_t gfp_flags, int migratetype)
{
	...
	if (page) {
		...
		pram_zone_statistics(preferred_zone, zone);
	}
	local_irq_restore(flags);
	return page;
}
...
#endif
		if (!page)
			page = __rmqueue_pram(zone, order, migratetype);
	} while (page && check_new_pages(page, order));
	spin_unlock(&zone->lock);
	if (!page)
		goto failed;
	__mod_pram_zone_freepage_state(zone, -(1 << order),
...
#endif	/* CONFIG_NUMA */
static void wake_all_mbs_mntrds(unsigned int order, const struct alloc_context *ac)
{
	struct zoneref *z;
	struct zone *zone;
	...
	for_each_zone_zonelist_nodemask(zone, z, ac->zonelist,
		...
		if (last_pgdat != zone->zone_pgdat)
			wakeup_mbs_mntrd(zone, order, ac->high_zoneidx);
...
//$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
		...
		if (ac->spread_dirty_pages) { //__GFP_WRITE
			...
			if (!pram_node_threshold_ok(zone->zone_pgdat)) {
...
/******************************************************/
			...
			default:
				...
				if (pram_zone_watermark_ok(zone, order, mark,
						ac_classzone_idx(ac), alloc_flags))
					goto try_this_zone;
...
static struct page *
__alloc_prams_direct_compact(gfp_t gfp_mask, unsigned int order,
...
static inline struct page *
__alloc_prams_direct_reclaim(gfp_t gfp_mask, unsigned int order,
		unsigned int alloc_flags, const struct alloc_context *ac,
...
static inline struct page *
__alloc_prams_slowpath(gfp_t gfp_mask, unsigned int order,
...
//		wake_all_mbs_mntrds(order, ac);
	...
	if (can_direct_reclaim &&
		...
		page = __alloc_prams_direct_compact(gfp_mask, order,
						alloc_flags, ac,
...
static inline bool prepare_alloc_prams(gfp_t gfp_mask, unsigned int order,
		...
		unsigned int *alloc_flags)
{
	ac->high_zoneidx = gfp_zone(gfp_mask); /*GFP_PRAM*/
	ac->zonelist = node_zonelist(preferred_nid, gfp_mask);
	ac->nodemask = nodemask; /*NULL*/
	ac->migratetype = gfpflags_to_migratetype(gfp_mask);/*0*/
...
void __meminit get_pfn_range_for_nid_pram(unsigned int nid,
		unsigned long *start_pfn, unsigned long *end_pfn)
{
	unsigned long this_start_pfn, this_end_pfn;
	int i;
	...
	*end_pfn = 0;
	...
	for_each_pram_pfn_range(i, nid, &this_start_pfn, &this_end_pfn, NULL) {
		*start_pfn = min(*start_pfn, this_start_pfn);
		*end_pfn = max(*end_pfn, this_end_pfn);
	}
...
static void __meminit calculate_node_totalpages_pram(struct pglist_data *pgdat,
		unsigned long node_start_pfn,
		unsigned long node_end_pfn,
		unsigned long node_start_pfn_pram,
		unsigned long node_end_pfn_pram,
		unsigned long *zones_size,
		unsigned long *zholes_size)
{
	unsigned long realtotalpages = 0, totalpages = 0;
	...
	for (i = 0; i < MAX_NR_ZONES; i++) {
		...
		if (i != ZONE_PRAM){
			...
			size = zone_spanned_pages_in_node(pgdat->node_id, i,
					...
					zones_size);
			real_size = size - zone_absent_pages_in_node(pgdat->node_id, i,
					node_start_pfn_pram, node_end_pfn_pram,
					zholes_size);
		}
		if (size)
			zone->zone_start_pfn = zone_start_pfn;
...
static void __paginginit free_area_init_core(struct pglist_data *pgdat)
{
	...
	init_waitqueue_head(&pgdat->mbs_mntrd_wait);
#ifdef CONFIG_COMPACTION
		...
		unsigned long node_start_pfn, unsigned long *zholes_size)
{
	...
	//<<<2018.02.14 Yongseob
	unsigned long start_pfn_pram = 0;
	unsigned long end_pfn_pram = 0;
	//>>>
...
//<<<2018.02.14 Yongseob
/* Find the lowest pfn of pram for a node */
static unsigned long __init find_min_pfn_for_node_pram(int nid)
{
	unsigned long min_pfn = ULONG_MAX;
	unsigned long start_pfn;
	int i;

	for_each_pram_pfn_range(i, nid, &start_pfn, NULL, NULL)
		min_pfn = min(min_pfn, start_pfn);

	if (min_pfn == ULONG_MAX) {
		pr_warn("Could not find start_pfn of pram for node %d\n", nid);
		return 0;
	}
...
/**
 * find_min_pfn_with_active_regions_pram - Find the minimum PFN(pram) registered
 *
 * It returns the minimum PFN based on information provided via
 * memblock_set_node().
 */
unsigned long __init find_min_pfn_with_active_regions_pram(void)
{
	return find_min_pfn_for_node_pram(MAX_NUMNODES);
}
//>>>
...
void __init free_area_init_nodes(unsigned long *max_zone_pfn)
{
	...
	//<<<2018.02.14 Yongseob
	start_pfn_pram = find_min_pfn_with_active_regions_pram();
	//>>>
	...
	for (i = 0; i < MAX_NR_ZONES; i++) {
		...
		//<<<2018.02.14 Yongseob
		if (i == ZONE_PRAM){
			end_pfn_pram = max(max_zone_pfn[i], start_pfn_pram);
			arch_zone_lowest_possible_pfn[i] = start_pfn_pram;
			arch_zone_highest_possible_pfn[i] = end_pfn_pram;
