...
#include <linux/mutex.h> /*MBS-counter*/
...
static struct kmem_cache *mbsfs_pram_cache;
...
static int mpol_set_nodemask_pram(struct mempolicy *pol,
		     const nodemask_t *nodes, struct nodemask_scratch *nsc)
{
	int ret;
...
struct mempolicy *__get_vma_pram_policy(struct vm_area_struct *vma,
						unsigned long addr)
{
	struct mempolicy *pol = NULL;
...
static int apply_pram_policy_zone(struct mempolicy *policy, enum zone_type zone)
{
	enum zone_type dynamic_policy_zone = pram_policy_zone;
...
static nodemask_t *pram_policy_nodemask(gfp_t gfp, struct mempolicy *policy)
{
	...
	if (unlikely(policy->mode == MPOL_BIND) &&
			apply_pram_policy_zone(policy, gfp_zone(gfp)) &&
...
int huge_pram_node(struct vm_area_struct *vma, unsigned long addr, gfp_t gfp_flags,
				struct mempolicy **mpol, nodemask_t **nodemask)
{
	int nid;

	*mpol = get_vma_pram_policy(vma, addr);
	*nodemask = NULL;	/* assume !MPOL_BIND */
...
mbsfs_lookup(struct mbsfs_policy *sp, unsigned long start, unsigned long end)
{
	...
	while (n) {
		struct mbsfs_pram_node *p = rb_entry(n, struct mbsfs_pram_node, nd);
	...
	if (!n)
		return NULL;
	for (;;) {
		struct mbsfs_pram_node *w = NULL;
		struct rb_node *prev = rb_prev(n);
		if (!prev)
			break;
		w = rb_entry(prev, struct mbsfs_pram_node, nd);
		if (w->end <= start)
			break;
		n = prev;
	}
	return rb_entry(n, struct mbsfs_pram_node, nd);
}
/*
...
static void mbsfs_insert(struct mbsfs_policy *sp, struct mbsfs_pram_node *new)
{
	struct rb_node **p = &sp->root.rb_node;
	struct rb_node *parent = NULL;
	struct mbsfs_pram_node *nd;

	while (*p) {
		parent = *p;
		nd = rb_entry(parent, struct mbsfs_pram_node, nd);
		if (new->start < nd->start)
			p = &(*p)->rb_left;
		else if (new->end > nd->end)
			p = &(*p)->rb_right;
...
static void mbsfs_pram_free(struct mbsfs_pram_node *n)
{
	mpol_put_pram(n->policy);
	kmem_cache_free(mbsfs_pram_cache, n);
}
/**
		...
		struct mempolicy *pol)
{
	...
	n = kmem_cache_alloc(mbsfs_pram_cache, GFP_KERNEL);
	if (!n)
		return NULL;
...
err_out:
	...
	if (n_new)
		kmem_cache_free(mbsfs_pram_cache, n_new);
...
alloc_new:
	write_unlock(&sp->lock);
	ret = -ENOMEM;
	n_new = kmem_cache_alloc(mbsfs_pram_cache, GFP_KERNEL);
	if (!n_new)
		goto err_out;
...
void mpol_mbsfs_policy_init(struct mbsfs_policy *sp, struct mempolicy *mpol)
{
	...
	if (mpol) {
		...
		task_lock(current);
		ret = mpol_set_nodemask_pram(new, &mpol->w.user_nodemask, scratch);
		task_unlock(current);
		if (ret)
			goto put_new;
...
free_scratch:
		NODEMASK_SCRATCH_FREE(scratch);
put_mpol:
		mpol_put_pram(mpol);	/* drop our incoming ref on sb mpol */
	}
}
//<<<2018.05.18 Yongseob
