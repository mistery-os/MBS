...
#endif
//<<<2018.02.13 Yongseob
static struct memblock_region memblock_pram_init_regions[INIT_PRAM_REGIONS] __initdata_memblock;
//>>>
struct memblock memblock __initdata_memblock = {
	...
	//<<<2018.02.13 Yongseob
	.pram.regions		= memblock_pram_init_regions,
	.pram.cnt		= 1,	/* empty dummy entry */
	.pram.max		= INIT_PRAM_REGIONS,
	.pram.name		= "pram",
	//>>>
					...
					phys_addr_t base, phys_addr_t size)
{
	...
	for (i = 0; i < type->cnt; i++)
		if (memblock_addrs_overlap(base, size, type->regions[i].base,
					   type->regions[i].size))
			break;
	pr_info("memblock_overlaps_region i=%d, type->cnt=%d\n",i,type->cnt);
	return i < type->cnt;
}
...
//<<<2018.02.13 Yongseob
int __init_memblock memblock_add_node_pram(phys_addr_t base, phys_addr_t size,
		int nid)
{
	return memblock_add_range(&memblock.pram, base, size, nid, 0);
}

int __init_memblock memblock_add_pram(phys_addr_t base, phys_addr_t size)
{
	phys_addr_t end = base + size - 1;
 ...
 */
void __init_memblock __next_pfn_range(int *idx, int *ix, int nid, 
...
//<<<2018.02.13 Yongseob
void __init_memblock __next_pram_pfn_range(int *idx, int nid,
		unsigned long *out_start_pfn,
		unsigned long *out_end_pfn, int *out_nid)
{
...
//<<<2018.02.13 Yongseob
void __init_memblock memblock_trim_pram(phys_addr_t align)
{
	phys_addr_t start, end, orig_start, orig_end;
	struct memblock_region *r;

	for_each_memblock(pram, r) {
		orig_start = r->base;
		orig_end = r->base + r->size;
		start = round_up(orig_start, align);
		end = round_down(orig_end, align);
		...
		if (start < end) {
			...
			memblock_remove_region(&memblock.pram,
					r - memblock.pram.regions);
			r--;
		}
	}
}
